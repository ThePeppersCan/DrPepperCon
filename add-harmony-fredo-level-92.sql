-- REPO COMPANY: HARMONY LEVEL 92 FREDO + LEVEL 99 PET SKILLCAPE
...
-- Give shared Harmony rewards to every existing account once their levels are reached.
update public.characters c
set bank_items = jsonb_set(coalesce(c.bank_items, '{}'::jsonb), '{pet_fredo}', '1'::jsonb, true)
where coalesce((select count from public.counter where id = 1), 0) >= 6517253
  and coalesce((c.bank_items->>'pet_fredo')::integer, 0) < 1;

-- Give the level 99 skillcape.
update public.characters c
...
begin
  ...
  if new_count >= 6517253 then
    update public.characters c
       set bank_items = jsonb_set(coalesce(c.bank_items, '{}'::jsonb), '{pet_fredo}', '1'::jsonb, true)
     where coalesce((c.bank_items->>'pet_fredo')::integer, 0) < 1;
  end if;
