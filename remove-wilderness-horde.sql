-- REPO COMPANY: COMPLETE WILDERNESS HORDE REMOVAL
-- Run this entire file once in Supabase -> SQL Editor.
--
-- Removes only Wilderness Horde scores/support and restores the three original
-- Endless Horde maps. Normal solo/multiplayer Horde scores, fighter designs,
-- GP, bank items and all other account progress are preserved.

begin;

-- Remove Wilderness-only leaderboard entries.
delete from public.endless_horde_scores
where map_id = 'zombie-wilderness';

delete from public.multiplayer_horde_scores
where map_id = 'zombie-wilderness';

-- Restore the original multiplayer map restriction.
...
alter table public.multiplayer_horde_scores
  ...
  check (map_id in ('zombie-varrock','zombie-falador','zombie-morytania'));

-- Legacy four-argument scorer.
...
begin
  ...
  if p_map_id not in ('zombie-varrock','zombie-falador','zombie-morytania') then
...
-- Five-argument scorer used by fallback clients.
create or replace function public.submit_endless_horde_score(
  p_map_id text,
  p_wave integer,
  p_kills integer,
...
-- Full run-summary scorer used by the restored website.
...
-- Restore the original three-map leaderboard readers.
...
as $$
  ...
  where s.map_id in ('zombie-varrock','zombie-falador','zombie-morytania')
...
-- Restore multiplayer scoring to the original three maps.
...
-- Restore permissions and refresh PostgREST's schema cache.
