export class RunManager {
  ...
  sanitizeProfile(raw){
    ...
    p.settings=this.settingsManager.sanitize(p.settings);p.stats=p.stats||{};p.achievements=Array.isArray(p.achievements)?p.achievements:[];if(p.activeRun)this.sanitizeRun(p.activeRun);return p;
  }
  sanitizeRun(run){
    ...
    run.flags.tutorialRun=Boolean(run.flags.tutorialRun);run.history=Array.isArray(run.history)?run.history:[];return run;
  }
  ...
  newRun(profile,manager='Manager'){
    ...
    let ordered=regular;
    if(tutorial){const intro=regular.find(o=>o.id==='fitchburg')||regular[0];ordered=[intro,...regular.filter(o=>o.id!==intro.id)];}
  ...
  finishMatch(run){
    ...
    run.history.push({week:run.week,opponentId:m.opponentId,won:m.won,player:m.playerScore,ai:m.aiScore,combo:m.metrics.comboBest,laneWins:(m.log||[]).reduce((n,x)=>n+Number(x.laneSummary?.won||0),0),snitch:Math.round(m.snitchMeter||0)});
    run.pendingReward={type:'card',options:this.draftOptions(run)};
    // Relics arrive after the player has learned cards, positioning and combos.
    if(run.week===2||run.week===4)run.pendingReward={type:'relic',options:this.relicOptions(run)};
  ...
  #deckTagCounts(run){
    const counts={};for(const inst of run.deck||[]){const c=this.data.cardById[inst.id];for(const tag of c?.tags||[])if(!['player','tactic','legendary'].includes(tag))counts[tag]=(counts[tag]||0)+1;}return counts;
  }
  #pickOne(list,rng,used){const pool=list.filter(c=>!used.has(c.id));if(!pool.length)return null;const c=pool[Math.floor(rng()*pool.length)];used.add(c.id);return c.id;}
  draftOptions(run){
    const rng=this.random.rng(run.seed,`draft:${run.week}`),offerPenalty=run.flags.rewardOfferPenalty||0;run.flags.rewardOfferPenalty=0;
    const allowed=new Set(run.unlockedCards||this.data.starterUnlocked),pool=this.data.cards.filter(c=>allowed.has(c.id)),counts=this.#deckTagCounts(run),dominant=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([t])=>t),used=new Set(),result=[];
    const fit=pool.filter(c=>(c.tags||[]).some(t=>dominant.includes(t)));
    const bridge=pool.filter(c=>(c.tags||[]).some(t=>['control','flying','beater','pass','support','crowd','trick'].includes(t))&&!(c.tags||[]).some(t=>dominant.includes(t)));
    const wild=pool.filter(c=>c.rarity==='Rare'||(c.tags||[]).includes('legendary')||!(run.deck||[]).some(x=>x.id===c.id));
    for(const group of [fit,bridge,wild,pool]){const id=this.#pickOne(group,rng,used);if(id)result.push(id);if(result.length>=Math.max(2,3-offerPenalty))break;}
    while(result.length<Math.max(2,3-offerPenalty)){const id=this.#pickOne(pool,rng,used);if(!id)break;result.push(id);}return result;
  }
  relicOptions(run){
    const rng=this.random.rng(run.seed,`relic:${run.week}`),allowed=new Set(run.unlockedRelics||[]);let pool=this.data.relics.filter(r=>allowed.has(r.id)&&!run.relics.includes(r.id));if(pool.length<3)pool=[...pool,...this.data.relics.filter(r=>!run.relics.includes(r.id)&&!pool.includes(r)).slice(0,3-pool.length)];for(let i=pool.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]];}return pool.slice(0,3).map(r=>r.id);
  }
  takeReward(run,choiceId,profile){
    if(!run.pendingReward)return false;const rewardType=run.pendingReward.type;
  ...
  advanceAfterReward(run){
    ...
    const eventWeeks=run.flags?.tutorialRun?[2]:[0,2,4];
    if(eventWeeks.includes(completedWeek)){const rng=this.random.rng(run.seed,`event:${completedWeek}`);run.pendingEvent=this.clone(this.data.events[Math.floor(rng()*this.data.events.length)]);this.stateMachine.transition(run,'phase','event',{force:true});}
    else this.stateMachine.transition(run,'phase','hub',{force:true});
  }
  ...
  grantMetaUnlocks(profile,run){
    profile.runs=(profile.runs||0)+1;if(run.champion)profile.championships=(profile.championships||0)+1;profile.bestSeasonWins=Math.max(profile.bestSeasonWins||0,run.wins||0);profile.tutorialVersion=Math.max(Number(profile.tutorialVersion||0),3);
    const rng=this.random.rng(run.seed,`meta:${profile.runs}`),pick=a=>a.length?a[Math.floor(rng()*a.length)]:null,lockedCards=this.data.cards.filter(c=>!profile.unlockedCards.includes(c.id)),lockedRelics=this.data.relics.filter(r=>!profile.unlockedRelics.includes(r.id)),unlocked=[];
    if(lockedCards.length){const c=pick(lockedCards);profile.unlockedCards.push(c.id);profile.discoveredCards.push(c.id);unlocked.push({type:'card',id:c.id,name:c.name});}else if(lockedRelics.length){const r=pick(lockedRelics);profile.unlockedRelics.push(r.id);profile.discoveredRelics.push(r.id);unlocked.push({type:'relic',id:r.id,name:r.name});}else{profile.legacy=(profile.legacy||0)+1;unlocked.push({type:'legacy',name:`Director Legacy ${profile.legacy}`});}
    if(run.champion){const remaining=this.data.relics.filter(r=>!profile.unlockedRelics.includes(r.id));const r=pick(remaining);if(r){profile.unlockedRelics.push(r.id);profile.discoveredRelics.push(r.id);unlocked.push({type:'relic',id:r.id,name:r.name});}}
    profile.updatedAt=Date.now();profile.lastRun={seed:run.seed,wins:run.wins,losses:run.losses,champion:run.champion,at:Date.now()};return unlocked;
  }
