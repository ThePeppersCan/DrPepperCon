(() => {
  ...
  function aimPointer(event) {
    ...
    pointer.x = (event.clientX - rect.left) * (R.W / rect.width);
    pointer.y = (event.clientY - rect.top) * (R.H / rect.height);
    pointer.active = true;
  }
  ...
  function drawLauncher(){const a=Math.atan2(pointer.y-game.launcher.y,pointer.x-game.launcher.x);ctx.save();ctx.translate(game.launcher.x,game.launcher.y);ctx.rotate(a-Math.PI/2);ctx.fillStyle='#2d261d';ctx.strokeStyle='#e0b64e';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-18,5);ctx.lineTo(0,34);ctx.lineTo(18,5);ctx.lineTo(10,-12);ctx.lineTo(-10,-12);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();ctx.beginPath();ctx.arc(game.launcher.x,game.launcher.y,12,0,Math.PI*2);ctx.fillStyle='#d9edf5';ctx.shadowColor='#86ddff';ctx.shadowBlur=14;ctx.fill();ctx.shadowBlur=0;}
  ...
  function simulateTrajectory(maxSteps,extended=false){let dx=pointer.x-game.launcher.x,dy=Math.max(70,pointer.y-game.launcher.y),mag=Math.hypot(dx,dy)||1,x=game.launcher.x,y=game.launcher.y+15,vx=dx/mag*610,vy=dy/mag*610;const points=[];for(let i=0;i<maxSteps;i++){vy+=520*(1/120);x+=vx*(1/120);y+=vy*(1/120);if(x<16||x>884){vx*=-.88;x=clamp(x,16,884);if(!extended&&i>6)break;}if(y<84){vy=Math.abs(vy)*.86;if(!extended&&i>6)break;}let hit=false;for(const peg of game.pegs){if(peg.removed)continue;const dd=Math.hypot(x-peg.x,y-peg.y);if(dd<8+peg.r){const nx=(x-peg.x)/(dd||1),ny=(y-peg.y)/(dd||1),dot=vx*nx+vy*ny;vx-=1.88*dot*nx;vy-=1.88*dot*ny;hit=true;break;}}points.push({x,y});if(hit&&!extended)break;if(y>560)break;}return points;}
  ...
  function resizeCanvas(){if(!canvas)return;const shell=canvas.parentElement;const ratio=R.W/R.H;const maxW=shell.clientWidth;canvas.style.width=`${maxW}px`;canvas.style.height=`${maxW/ratio}px`;}
