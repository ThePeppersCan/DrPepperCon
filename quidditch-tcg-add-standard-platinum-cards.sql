...
drop function if exists public.get_my_quidditch_tcg_collection();
create function public.get_my_quidditch_tcg_collection()
...
drop function if exists public.get_public_quidditch_tcg_collection(text);
create function public.get_public_quidditch_tcg_collection(p_username text)
returns table(username text, cards text[], card_count integer, total_cards integer)
language sql security definer set search_path=public as $$
  select c.username,
         coalesce(q.cards,'{}'::text[]) as cards,
...
drop function if exists public.buy_quidditch_tcg_pack();
create function public.buy_quidditch_tcg_pack()
...
drop function if exists public.open_quidditch_tcg_pack();
create function public.open_quidditch_tcg_pack()
returns table(
  card_id text,
  owned_cards text[],
  bank_items jsonb,
...
begin
  ...
  -- Four duplicate-proof rarity pools.
  -- Legendary: 4%, Platinum: 8%, Full Art: 29%, Standard: remaining chance.
  declare
  ...
  begin
    ...
    from unnest(v_available) card where card not like '%_full_art' and card not like '%_platinum';

    if cardinality(v_legendary_available)>0 and v_roll<0.04 then
      v_card:=v_legendary_available[1+floor(random()*cardinality(v_legendary_available))::integer];
    elsif cardinality(v_platinum_available)>0 and v_roll<0.12 then
      v_card:=v_platinum_available[1+floor(random()*cardinality(v_platinum_available))::integer];
    elsif cardinality(v_full_art_available)>0 and v_roll<0.4133 then
