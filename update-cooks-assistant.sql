-- REPO COMPANY: COOK'S ASSISTANT MINI QUEST + COOKING SKILL
-- Run once in Supabase -> SQL Editor. Preserves all existing accounts, XP, pets, GP and bank items.

alter table public.characters add column if not exists cooking_xp integer not null default 0 check (cooking_xp >= 0);
alter table public.characters add column if not exists quest_state jsonb not null default '{}'::jsonb;
alter table public.characters add column if not exists completed_quests text[] not null default '{}'::text[];

create or replace function public.get_cooks_assistant_state()
returns table(
 ...
 has_flour boolean, completed boolean, cooking_xp integer, gp bigint
)
language sql security definer set search_path=public as $$
 select
   case when 'cooks_assistant'=any(coalesce(c.completed_quests,'{}'::text[])) then 'completed'
        else coalesce(nullif(c.quest_state->>'status',''),'not_started') end,
   ...
   coalesce(c.cooking_xp,0),coalesce(c.gp,0)
...
declare c public.characters%rowtype; st jsonb; loc text; done boolean;
begin
 ...
 if p_action='reset' then
   if done then raise exception 'Completed quests cannot be reset'; end if;
   st:='{}'::jsonb;
 ...
 if p_action='start' then
   if not done and coalesce(st->>'status','')<>'active' then
     st:=jsonb_build_object('status','active','location','kitchen','has_bucket',false,'has_pot',false,'has_egg',false,'has_milk',false,'has_grain',false,'hopper_loaded',false,'lever_pulled',false,'has_flour',false,'started_at',now());
     update public.characters set quest_state=st where id=c.id;
   end if;
   return query select * from public.get_cooks_assistant_state(); return;
 end if;
 if done then return query select * from public.get_cooks_assistant_state(); return; end if;
 ...
 if p_action like 'travel_%' then
   ...
   if not coalesce((st->>'has_egg')::boolean,false) or not coalesce((st->>'has_milk')::boolean,false) or not coalesce((st->>'has_flour')::boolean,false) then raise exception 'You do not have all three ingredients'; end if;
   st:=jsonb_set(st,'{status}',to_jsonb('completed'::text),true);
   update public.characters as ch
      set quest_state=st,
          cooking_xp=ch.cooking_xp+300,
          gp=ch.gp+500,
          completed_quests=array_append(ch.completed_quests,'cooks_assistant')
    where ch.id=c.id;
   return query select * from public.get_cooks_assistant_state(); return;
 else raise exception 'Unknown quest action';
 end if;
 update public.characters set quest_state=st where id=c.id;
...
-- Include Cooking in account profiles and total level.
drop function if exists public.get_my_character();
create function public.get_my_character() returns table(username text,woodcutting_xp integer,mining_xp integer,fishing_xp integer,agility_xp integer,slayer_xp integer,attack_xp integer,strength_xp integer,defence_xp integer,sailing_xp integer,runecrafting_xp integer,cooking_xp integer,agility_best_ms integer,collection text[],created_at timestamptz)
language sql security definer set search_path=public as $$
 select c.username,c.woodcutting_xp,c.mining_xp,c.fishing_xp,c.agility_xp,c.slayer_xp,c.attack_xp,c.strength_xp,c.defence_xp,c.sailing_xp,c.runecrafting_xp,c.cooking_xp,c.agility_best_ms,c.collection,c.created_at from public.characters c where c.user_id=auth.uid() limit 1
$$;
...
language sql security definer set search_path=public as $$
 select c.username,c.woodcutting_xp,c.mining_xp,c.fishing_xp,c.agility_xp,c.slayer_xp,c.attack_xp,c.strength_xp,c.defence_xp,c.sailing_xp,c.runecrafting_xp,c.cooking_xp,c.agility_best_ms,c.collection,c.created_at from public.characters c where lower(c.username)=lower(p_username) limit 1
$$;
grant execute on function public.get_public_character(text) to anon,authenticated;

create or replace function public.get_leaderboard() returns table(username text,total_level integer)
language sql security definer set search_path=public as $$
 select c.username,(public.level_from_xp(c.woodcutting_xp)+public.level_from_xp(c.mining_xp)+public.level_from_xp(c.fishing_xp)+public.level_from_xp(c.agility_xp)+public.level_from_xp(c.slayer_xp)+public.level_from_xp(c.attack_xp)+public.level_from_xp(c.strength_xp)+public.level_from_xp(c.defence_xp)+public.level_from_xp(c.sailing_xp)+public.level_from_xp(c.runecrafting_xp)+public.level_from_xp(c.cooking_xp))::integer from public.characters c order by 2 desc,c.username asc limit 100
$$;
