-- Repairs the Shooting Star mining RPCs, including installations where an older
-- version of the functions already exists with a different return type.
...
begin
  ...
  if c.mining_afk_active and c.mining_afk_last_click_at is not null and c.mining_afk_gp_checkpoint_at is not null then
    ...
    if v_end>c.mining_afk_gp_checkpoint_at then
      v_periods:=floor(extract(epoch from(v_end-c.mining_afk_gp_checkpoint_at))/600);
    end if;
    if v_periods>0 then
      update public.characters
      set gp=coalesce(public.characters.gp,0)+v_periods*1000,
          mining_afk_gp_checkpoint_at=c.mining_afk_gp_checkpoint_at+(v_periods*interval '10 minutes')
  ...
  return query
  select coalesce(c.mining_xp,0),coalesce(c.gp,0),c.active_pet,v_pet_name,c.mining_afk_active,
         case when c.mining_afk_active and c.mining_afk_last_click_at is not null
              then greatest(0,ceil(420-extract(epoch from(v_now-c.mining_afk_last_click_at)))::integer)
              else 0 end,
         v_periods*1000,0;
end $$;
...
begin
  ...
  update public.characters
  set mining_xp=coalesce(public.characters.mining_xp,0)+v_gain,
      gp=coalesce(public.characters.gp,0)+v_periods*1000,
      mining_afk_active=true,
      mining_afk_last_click_at=v_now,
      mining_afk_gp_checkpoint_at=case
        when public.characters.mining_afk_gp_checkpoint_at is null or not c.mining_afk_active then v_now
        else public.characters.mining_afk_gp_checkpoint_at+(v_periods*interval '10 minutes')
      end
  where user_id=auth.uid()
  returning * into c;
