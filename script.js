...
const COMBAT_SHADOW_WEAPON_IMAGE = new Image();
COMBAT_SHADOW_WEAPON_IMAGE.src = 'assets/combat/tumekens-shadow-weapon.png';
...
function updateCombat(dt, now) {
  ...
  if (nearest && nearestD <= p.range && now - p.lastAttack >= p.attackRate * 1000) {
    ...
    if (s.weapon === 'sword') {
      ...
      p.shadowAttackAngle = Math.atan2(nearest.y - p.y, nearest.x - p.x);
      damageCombatEnemy(nearest, p.damage);
...
function drawCombatPlayer(ctx,p,weapon){
  ...
  if(weapon==='bow'){
    ...
    if(COMBAT_SHADOW_WEAPON_IMAGE.complete && COMBAT_SHADOW_WEAPON_IMAGE.naturalWidth){
      ctx.save();ctx.translate(18,-5);ctx.rotate(-0.18);ctx.drawImage(COMBAT_SHADOW_WEAPON_IMAGE,-14,-35,31,70);ctx.restore();
    }else{
...
const BANK_ITEM_DEFS={
  ...
  repo_combat_rare_drop:{name:'Rare Drop',image:'assets/rare-drop/chest-shake-01.png',description:"A mysterious purple chest. You'll be able to open it soon…"}
};
const REPO_RARE_DROP_FRAMES=Array.from({length:6},(_,i)=>`assets/rare-drop/chest-shake-${String(i+1).padStart(2,'0')}.png`);
let repoRareDropAnimationTimer=null;
function bankStandardItemSlot(id,qty){const d=BANK_ITEM_DEFS[id];if(!d)return null;if(id==='repo_combat_rare_drop')return `<div class="bank-slot graceful-bank-slot repo-rare-drop-slot"><img src="${REPO_RARE_DROP_FRAMES[0]}" alt="Rare Drop chest" class="bank-item-art repo-rare-drop-art"><b>Rare Drop</b><small>A mysterious purple chest.<br><em>Soon…</em></small><strong>${Number(qty).toLocaleString('en-GB')}</strong></div>`;return `<div class="bank-slot graceful-bank-slot"><img src="${d.image}" alt="${d.name}" class="bank-item-art"><b>${d.name}</b><small>${d.description}</small><strong>${Number(qty).toLocaleString('en-GB')}</strong></div>`}
function startRepoRareDropBankAnimation(){clearInterval(repoRareDropAnimationTimer);let frame=0;const arts=[...document.querySelectorAll('.repo-rare-drop-art')];if(!arts.length)return;repoRareDropAnimationTimer=setInterval(()=>{frame=(frame+1)%REPO_RARE_DROP_FRAMES.length;arts.forEach(img=>img.src=REPO_RARE_DROP_FRAMES[frame])},115)}
...
function qmEnsurePurpleDropTestButton(){
  const panel=$('qmAdminSpecialTester');
  if(!panel||document.getElementById('qmTestPurpleDrop'))return;
  const button=document.createElement('button');
  button.type='button';
  button.id='qmTestPurpleDrop';
  button.textContent='TEST PURPLE DROP';
  button.title='Preview the Repo Combat rare-drop notification';
  button.addEventListener('click',()=>{
    if(!character||String(character.username||'').toLowerCase()!=='catasthma'||!toaState.adminMode)return;
    showRepoPurpleDropNotice();
  });
  const allButton=panel.querySelector('#qmTestAllSpecials');
  if(allButton?.parentElement)allButton.parentElement.appendChild(button);
...
function qmUpdateAdminSpecialTester(){
  ...
  if(isCat&&toaState.adminMode)qmEnsurePurpleDropTestButton();
}
...
setTimeout(qmEnsurePurpleDropTestButton,0);
...
const qmCommentaryTemplates={
  ...
  penaltyAwarded:[
    ({pet,team})=>`Barry Bramble: Whistle! ${pet} has been fouled — penalty to ${team}!`,
    ({pet,team})=>`Barry Bramble: That's a clear foul on ${pet}. ${team} have a penalty!`,
    ({team})=>`Barry Bramble: The referee points to the spot — penalty for ${team}!`
  ],
  penaltyScored:[
    ({pet,team})=>`Barry Bramble: ${pet} buries the penalty for ${team}!`,
    ({pet})=>`Barry Bramble: Cool as you like — ${pet} converts from the spot!`
  ],
  penaltyMissed:[
    ({pet})=>`Barry Bramble: ${pet} cannot convert the penalty!`,
    ({pet})=>`Barry Bramble: The penalty is missed — agony for ${pet}!`
  ],
  snitchAppeared:[
    ()=>`Barry Bramble: The Golden Snitch has appeared! Every player has abandoned the Quaffle!`,
    ()=>`Barry Bramble: There it is — the Golden Snitch is loose on the pitch!`
  ],
  snitchCaught:[
    ({pet,team})=>`Barry Bramble: ${pet} has caught the Golden Snitch! ${team} win the match!`,
    ({pet})=>`Barry Bramble: Caught! ${pet} gets the Snitch and ends the game!`
  ],
  fulltime:[
...
// Barry event-sprite alignment and cleanup.
(function(){
  const style=document.createElement('style');
  style.textContent=`
    #qmCommentatorStudio.is-set-piece #qmCommentatorSprite,
    #qmCommentatorStudio.is-shocked #qmCommentatorSprite,
    #qmCommentatorStudio.is-nail-biting #qmCommentatorSprite {
      left:50% !important;
...
// Force the tiny Barry fly-by Snitch to ignore generic commentator image sizing.
(function(){
  if(document.getElementById('qmBarryTinySnitchStyle'))return;
  const style=document.createElement('style');
  style.id='qmBarryTinySnitchStyle';
  style.textContent=`#qmCommentatorStudio img.qm-barry-snitch-flyby{width:18px!important;max-width:18px!important;min-width:18px!important;height:auto!important;object-fit:contain!important;}`;
  document.head.appendChild(style);
})();

// Golden Snitch fly-bys inside Barry Bramble's commentator card.
(function(){
  let timers=[];
  const clear=()=>{
    timers.forEach(clearTimeout);timers=[];
    document.querySelectorAll('#qmCommentatorStudio .qm-barry-snitch-flyby').forEach(el=>el.remove());
  };
  function fly({direction=1,delay=0,behind=false,top=36}){
    timers.push(setTimeout(()=>{
      const studio=document.getElementById('qmCommentatorStudio');
      if(!studio||studio.dataset.setPiece!=='snitch')return;
      const el=document.createElement('img');
      el.className='qm-barry-snitch-flyby';
      el.src='assets/quidditch-golden-snitch.png';
      el.alt='Golden Snitch';
      const start=direction>0?'-28px':'calc(100% + 8px)';
      const end=direction>0?'calc(100% + 10px)':'-30px';
      Object.assign(el.style,{
        position:'absolute',zIndex:behind?'1':'12',height:'auto',pointerEvents:'none',
        left:start,top:`${top}%`,opacity:'0',
        transform:`translateY(-50%) scaleX(${direction>0?1:-1})`,
        filter:'drop-shadow(0 0 2px rgba(255,210,45,.72))'
      });
      // The commentator card has broad image sizing rules, so force the fly-by
      // sprite to remain tiny instead of inheriting Barry's full-card dimensions.
      ...
      studio.style.position='relative';studio.style.overflow='hidden';studio.appendChild(el);
      const anim=el.animate([
        {left:start,opacity:0,transform:`translateY(-50%) scaleX(${direction>0?1:-1}) rotate(-5deg)`},
        {offset:.08,opacity:1},
        {offset:.48,opacity:1,transform:`translateY(-70%) scaleX(${direction>0?1:-1}) rotate(6deg)`},
        {offset:.92,opacity:1},
        {left:end,opacity:0,transform:`translateY(-35%) scaleX(${direction>0?1:-1}) rotate(-4deg)`}
      ],{duration:1150,easing:'cubic-bezier(.18,.72,.2,1)',fill:'forwards'});
      anim.onfinish=()=>el.remove();
    },delay));
  }
  window.qmStartBarrySnitchFlybys=function(durationMs=7000){
    clear();
    // First pass: in front of Barry, across head height, then fully out of frame.
    fly({direction:1,delay:900,behind:false,top:34});
    // Second pass: returns from the opposite side behind Barry, then exits fully.
    fly({direction:-1,delay:Math.min(Math.max(2800,durationMs-2600),4300),behind:true,top:43});
  };
  window.qmStopBarrySnitchFlybys=clear;
})();

// Animated Repo Sports commentator, synchronised to commentary and major set-piece events.
(function(){
  const talkFrames=['22','23','24','23','28','24'];
  const setPieceIntro=[
    'assets/barry-events/barry-01.png','assets/barry-events/barry-02.png','assets/barry-events/barry-03.png',
    'assets/barry-events/barry-04.png','assets/barry-events/barry-05.png','assets/barry-events/barry-06.png'
  ];
  const nailBiteLoop=[
    'assets/barry-events/barry-07.png','assets/barry-events/barry-08.png','assets/barry-events/barry-09.png',
    'assets/barry-events/barry-10.png','assets/barry-events/barry-11.png','assets/barry-events/barry-10.png',
    'assets/barry-events/barry-09.png','assets/barry-events/barry-08.png'
  ];
  let talkTimer=null,stopTimer=null,goalTimer=null,eventTimer=null,eventIntroTimer=null,frameIndex=0,setPieceActive=false;
  ...
  function setAsset(path){const el=sprite();if(el)el.src=path}
  function clearNormalTimers(){clearInterval(talkTimer);clearTimeout(stopTimer);clearTimeout(goalTimer);talkTimer=null;stopTimer=null;goalTimer=null}
  function clearSetPieceTimers(){clearInterval(eventTimer);clearTimeout(eventIntroTimer);eventTimer=null;eventIntroTimer=null}
  function stop(){
    clearNormalTimers();
    if(setPieceActive)return;
    const s=studio();if(s?.classList.contains('is-tipped'))return;
    if(s)s.classList.remove('is-speaking','is-goal','is-set-piece','is-shocked','is-nail-biting');
    setFrame('22');
  }
  window.qmAnimateCommentator=function(type,text=''){
    const s=studio(),img=sprite();if(!s||!img||s.classList.contains('is-tipped'))return;
    // Penalty/Snitch commentary still enters the log, but Barry remains in the
    // dedicated shocked/nail-biting sequence until the set piece resolves.
    if(setPieceActive&&type!=='goal')return;
    clearNormalTimers();clearSetPieceTimers();
    if(type==='goal'){
      setPieceActive=false;
      s.classList.remove('is-speaking','is-set-piece','is-shocked','is-nail-biting');
      s.classList.add('is-goal');setFrame('29');
      goalTimer=setTimeout(()=>{s.classList.remove('is-goal');window.qmAnimateCommentator('talk',text)},1800);return;
    }
    s.classList.remove('is-goal','is-set-piece','is-shocked','is-nail-biting');s.classList.add('is-speaking');frameIndex=0;setFrame(talkFrames[0]);
  ...
  window.qmStartBarrySetPiece=function(kind='event'){
    const s=studio(),img=sprite();if(!s||!img)return;
    clearNormalTimers();clearSetPieceTimers();
    setPieceActive=true;frameIndex=0;
    s.classList.remove('is-speaking','is-goal','is-tipped','is-nail-biting');
    s.classList.add('is-set-piece','is-shocked');
    const introHolds=[105,105,120,260,180,170];
    const playIntro=index=>{
      if(!setPieceActive)return;
      if(index>=setPieceIntro.length){
        s.classList.remove('is-shocked');s.classList.add('is-nail-biting');
        frameIndex=0;setAsset(nailBiteLoop[0]);
        eventTimer=setInterval(()=>{
          if(!setPieceActive)return;
          frameIndex=(frameIndex+1)%nailBiteLoop.length;setAsset(nailBiteLoop[frameIndex]);
        },145);
        return;
      }
      setAsset(setPieceIntro[index]);
      eventIntroTimer=setTimeout(()=>playIntro(index+1),introHolds[index]||145);
    };
    playIntro(0);
    s.dataset.setPiece=kind;
  };
  window.qmResolveBarrySetPiece=function(success=false,text=''){
    const s=studio();
    setPieceActive=false;clearSetPieceTimers();clearNormalTimers();
    if(s){s.classList.remove('is-set-piece','is-shocked','is-nail-biting');delete s.dataset.setPiece;}
    if(success){window.qmAnimateCommentator('goal',text);return;}
    // A missed penalty gets a brief worried recovery frame, then Barry returns idle.
    if(s&&!s.classList.contains('is-tipped')){
      s.classList.add('is-speaking');setAsset('assets/barry-events/barry-12.png');
      stopTimer=setTimeout(stop,850);
    }
  };
  window.qmCancelBarrySetPiece=function(){
    setPieceActive=false;clearSetPieceTimers();clearNormalTimers();
    const s=studio();if(s){s.classList.remove('is-set-piece','is-shocked','is-nail-biting');delete s.dataset.setPiece;}
    stop();
  };
...
let repoRareDropRollBusy=false;
function showRepoPurpleDropNotice(){
  let notice=document.getElementById('repoPurpleDropNotice');
  if(!notice){notice=document.createElement('div');notice.id='repoPurpleDropNotice';notice.innerHTML='<span>◆</span><b>You have received a purple drop!</b>';document.body.appendChild(notice);}
  notice.classList.remove('show');void notice.offsetWidth;notice.classList.add('show');
  clearTimeout(notice._hideTimer);notice._hideTimer=setTimeout(()=>notice.classList.remove('show'),1100);
}
async function tryRepoCombatRareDrop(){
  if(repoRareDropRollBusy||!character)return;repoRareDropRollBusy=true;
  try{
    const {data,error}=await db.rpc('roll_repo_combat_rare_drop');
    if(error){if(!/roll_repo_combat_rare_drop|schema cache|could not find/i.test(`${error.message||''} ${error.details||''}`))console.warn('Rare-drop roll failed:',error);return;}
    const row=Array.isArray(data)?data[0]:data;if(!row?.won)return;
    if(bankState){bankState.items=bankState.items||{};bankState.items.repo_combat_rare_drop=Number(row.quantity||Number(bankState.items.repo_combat_rare_drop||0)+1);if($('bankDialog')?.open)renderBank();}
    showRepoPurpleDropNotice();
  }catch(error){console.warn('Rare-drop roll failed:',error)}finally{repoRareDropRollBusy=false}
}
if(!document.getElementById('repoRareDropStyles')){const style=document.createElement('style');style.id='repoRareDropStyles';style.textContent=`
#repoPurpleDropNotice{position:fixed;left:50%;top:17%;z-index:100000;display:flex;align-items:center;gap:8px;transform:translate(-50%,-12px) scale(.96);opacity:0;pointer-events:none;padding:9px 14px;border:2px solid #8d58c8;background:linear-gradient(#251531,#120b18);box-shadow:0 0 0 2px #24142f,0 0 18px #9c55dc99;color:#ead9ff;font:700 13px Georgia,serif;transition:opacity .16s,transform .16s}#repoPurpleDropNotice span{color:#bd78ff;text-shadow:0 0 7px #c98aff}#repoPurpleDropNotice.show{opacity:1;transform:translate(-50%,0) scale(1)}.repo-rare-drop-slot{border-color:#7c4bad!important;box-shadow:inset 0 0 14px #5b278844}.repo-rare-drop-slot .repo-rare-drop-art{image-rendering:pixelated;object-fit:contain}.repo-rare-drop-slot em{color:#c291ef;font-style:normal;font-weight:800}`;document.head.appendChild(style)}
...
(() => {
  ...
  let repoHordeRunHover=null;
  let repoHordeRunHoverTimer=null;
  function repoNormaliseRunUpgrades(value){
  ...
  function repoHordeRunDuration(seconds){
    const total=Math.max(0,Number(seconds)||0),minutes=Math.floor(total/60),secs=Math.floor(total%60);
    return `${minutes}m ${String(secs).padStart(2,'0')}s`;
  }
  function repoEnsureHordeRunHover(){
    if(repoHordeRunHover?.isConnected)return repoHordeRunHover;
    repoHordeRunHover=document.createElement('aside');
    repoHordeRunHover.id='repoHordeRunHoverCard';
    repoHordeRunHover.className='repo-horde-run-hover-card';
    repoHordeRunHover.setAttribute('role','tooltip');
    // Popovers render in the browser's top layer, above the combat dialog and its backdrop.
    if('showPopover' in HTMLElement.prototype)repoHordeRunHover.setAttribute('popover','manual');
    repoHordeRunHover.addEventListener('pointerenter',()=>clearTimeout(repoHordeRunHoverTimer));
    repoHordeRunHover.addEventListener('pointerleave',repoScheduleHideHordeRunHover);
    document.body.appendChild(repoHordeRunHover);
    return repoHordeRunHover;
  }
  function repoHideHordeRunHover(){
    const card=repoHordeRunHover;if(!card)return;
    card.classList.remove('show');
    if(card.matches?.(':popover-open')){try{card.hidePopover()}catch(_e){}}
  }
  function repoScheduleHideHordeRunHover(){
    clearTimeout(repoHordeRunHoverTimer);
    repoHordeRunHoverTimer=setTimeout(repoHideHordeRunHover,90);
  }
  async function repoResolveHordeRunProfile(username){
    const key=typeof qmWatcherKey==='function'?qmWatcherKey(username):String(username||'').trim().toLowerCase();
    let profile=(typeof qmWatcherProfileCache!=='undefined'&&qmWatcherProfileCache.get)?qmWatcherProfileCache.get(key):null;
    if(profile?.equipped_watchcard_background!==undefined)return profile;
    try{
      const {data}=await db.rpc('get_watchcard_backgrounds',{p_usernames:[username]});
      const row=Array.isArray(data)?data[0]:null;
      if(row){
        profile={...(profile||{}),username:row.username||username,equipped_watchcard_background:row.equipped_watchcard_background||null};
        if(typeof qmWatcherProfileCache!=='undefined'&&qmWatcherProfileCache.set)qmWatcherProfileCache.set(key,profile);
      }
    }catch(_e){}
    return profile||{username,equipped_watchcard_background:null};
  }
  async function repoShowHordeRunHover(row,run){
    if(!row||!run)return;
    clearTimeout(repoHordeRunHoverTimer);
    const card=repoEnsureHordeRunHover();
    const profile=await repoResolveHordeRunProfile(run.username||'Player');
    const upgrades=repoNormaliseRunUpgrades(run.best_upgrades);
    const weapon=({greataxe:"Dharok's Greataxe",blowpipe:'Toxic Blowpipe',shadow:"Tumeken's Shadow"}[String(run.best_weapon||'').toLowerCase()]||'Unknown weapon');
    const upgradeHtml=upgrades.length
      ? upgrades.map(upgrade=>`<span class="${Number(upgrade.rare_count||0)>0?'golden':''}">${escapeHtml(upgrade.icon||'◆')} ${escapeHtml(upgrade.name||'Upgrade')} ×${Math.max(1,Number(upgrade.count||1))}${Number(upgrade.rare_count||0)>0?` <i>✦${Number(upgrade.rare_count)}</i>`:''}</span>`).join('')
      : '<em>No saved upgrade summary for this older run.</em>';
    const accountName=run.username||profile?.username||'Player';
    const avatar=typeof qmWatcherAvatarForName==='function'?qmWatcherAvatarForName(accountName):'assets/default-avatar.png';
    const backdropItem=typeof partyPeteItem==='function'?partyPeteItem(profile?.equipped_watchcard_background):null;
    const backdropStyle=backdropItem?.image?` style="--repo-run-backdrop:url('${escapeHtml(backdropItem.image)}')"`:'';
    card.innerHTML=`<div class="repo-horde-run-identity ${backdropItem?'has-backdrop':''}"${backdropStyle}><div class="repo-horde-run-identity-shade"></div><img src="${escapeHtml(avatar)}" alt="${escapeHtml(accountName)} avatar"><div><small>ACCOUNT</small><b>${escapeHtml(accountName)}</b><span>${backdropItem?escapeHtml(backdropItem.name):'Default backdrop'}</span></div></div><header><div><small>ENDLESS HORDE PERSONAL BEST</small><b>${escapeHtml(accountName)}</b></div><strong>W${Number(run.best_wave||0)}</strong></header><div class="repo-horde-run-hover-stats"><span><small>KILLS</small><b>${Number(run.best_kills||0)}</b></span><span><small>RUN LEVEL</small><b>${Number(run.best_run_level||1)}</b></span><span><small>TIME</small><b>${repoHordeRunDuration(run.best_seconds)}</b></span><span><small>WEAPON</small><b>${escapeHtml(weapon)}</b></span></div><div class="repo-horde-run-hover-upgrades"><small>UPGRADES USED${Number(run.best_rare_picks||0)>0?` • ${Number(run.best_rare_picks)} GOLDEN`:''}</small><div>${upgradeHtml}</div></div>`;
    const rect=row.getBoundingClientRect();
    if(card.hasAttribute('popover')&&!card.matches(':popover-open')){try{card.showPopover()}catch(_e){}}
    card.classList.add('show');
    card.style.left='0px';card.style.top='0px';
    const width=card.offsetWidth||330,height=card.offsetHeight||180,pad=10;
    let left=rect.right+10,top=rect.top-8;
    if(left+width>window.innerWidth-pad)left=rect.left-width-10;
  ...
  async function loadEndlessLeaderboard(){
    ...
    if(error&&(/get_endless_horde_leaderboard_v2|schema cache|could not find/i.test(`${error.message||''} ${error.details||''}`))){
      ({data,error}=await db.rpc('get_endless_horde_leaderboard'));
    }
    for(const [mapId,host] of Object.entries(hosts)){
      ...
      host.querySelectorAll('.repo-horde-run-row').forEach((row,index)=>{
        ...
        row.addEventListener('pointerenter',()=>repoShowHordeRunHover(row,row._repoHordeRun));
        row.addEventListener('pointerleave',repoScheduleHideHordeRunHover);
        row.addEventListener('focus',()=>repoShowHordeRunHover(row,row._repoHordeRun));
        row.addEventListener('blur',repoScheduleHideHordeRunHover);
      });
    }
  }
  ...
  finishCombat=async function(survived){
    ...
    for(const pick of (Array.isArray(s.hordeBuild?.picks)?s.hordeBuild.picks:[])){
      const key=String(pick.id||pick.name||'upgrade');
      let item=upgradeIndex.get(key);
      if(!item){item={id:key,name:String(pick.name||'Upgrade'),icon:String(pick.icon||'◆'),count:0,rare_count:0};upgradeIndex.set(key,item);upgradeGroups.push(item);}
      item.count+=1;if(pick.rare)item.rare_count+=1;
    }
    try{
      const scorePayload={p_map_id:z.map,p_wave:z.wave,p_kills:s.kills,p_seconds:Math.floor(s.elapsed),p_weapon:s.weapon,p_run_level:s.runLevel,p_rare_picks:Number(s.hordeBuild?.rarePicks||0),p_upgrades:upgradeGroups};
      let {error:scoreError}=await db.rpc('submit_endless_horde_score_v2',scorePayload);
      if(scoreError&&(/submit_endless_horde_score_v2|schema cache|could not find/i.test(`${scoreError.message||''} ${scoreError.details||''}`))){
        ({error:scoreError}=await db.rpc('submit_endless_horde_score',{p_map_id:z.map,p_wave:z.wave,p_kills:s.kills,p_seconds:Math.floor(s.elapsed),p_weapon:s.weapon}));
      }
      if(scoreError)console.warn('Endless Horde score save failed:',scoreError);
      loadEndlessLeaderboard();
    }catch(_e){}
    $('combatUpgrade').classList.add('hidden');$('combatIntro').classList.remove('hidden');$('combatStart').textContent='PLAY AGAIN';
...
/* === PANDA TEXT + PERSISTENT ENDLESS HORDE LOGO FINAL FIX === */
(()=>{
  function ensurePersistentHordeLogo(){
    const dialog=document.getElementById('combatDialog');
    const section=document.getElementById('endlessHordeSection');
    if(!dialog||!section)return false;

    const all=[...dialog.querySelectorAll('.repo-horde-logo-wrap')];
    let logo=section.querySelector(':scope > .repo-horde-logo-wrap')||section.querySelector('.repo-horde-logo-wrap')||all[0]||null;

    if(!logo){
      logo=document.createElement('div');
      logo.className='repo-horde-logo-wrap';
      logo.innerHTML='<img class="repo-horde-logo-image" src="assets/combat/repo-horde-mode-logo.png" alt="Repo Horde Mode">';
    }

    const hiddenTabs=section.querySelector(':scope > .combat-mode-tabs');
    const head=section.querySelector(':scope > .endless-horde-head');
    if(logo.parentElement!==section){
      if(hiddenTabs)hiddenTabs.insertAdjacentElement('afterend',logo);
      else if(head)section.insertBefore(logo,head);
      else section.prepend(logo);
    }

    // Remove only genuine duplicate logo wrappers. Keep the one inside the
    // Endless Horde section so switching modes cannot destroy or orphan it.
    [...dialog.querySelectorAll('.repo-horde-logo-wrap')].forEach(node=>{
      if(node!==logo)node.remove();
    });
  ...
  const style=document.createElement('style');
  style.id='repoPersistentHordeLogoFinalStyles';
  style.textContent=`
    #quidditchModePitch .pet-label[data-nametag="nametag_panda_rare"] .qm-custom-nametag>b{
      top:calc(50% + 11px)!important;
      transform:translateY(-50%)!important;
    }
    #endlessHordeSection>.repo-horde-logo-wrap,
    #endlessHordeSection .repo-horde-logo-wrap{
  ...
  function scheduleEnsure(){
    ...
    requestAnimationFrame(()=>{
      scheduled=false;
      ensurePersistentHordeLogo();
    });
  }

  const start=()=>{
    if(!ensurePersistentHordeLogo()){
      let attempts=0;
      const timer=setInterval(()=>{
        attempts+=1;
        if(ensurePersistentHordeLogo()||attempts>=80)clearInterval(timer);
      },100);
    }
    const dialog=document.getElementById('combatDialog');
    if(dialog){
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest('[data-combat-menu],#endlessCombatMode,#standardCombatMode,#combatButton,#openCombat,#combatClose')){
      [0,40,140,350].forEach(delay=>setTimeout(ensurePersistentHordeLogo,delay));
    }
  });
})();

/* === PANDA LOWER TEXT + RESTORE HORDE LOGO FLAMES === */
(()=>{
  let flameTimer=null;
  let flameFrame=1;

  function restoreHordeFlames(){
    const section=document.getElementById('endlessHordeSection');
    const wrap=section?.querySelector('.repo-horde-logo-wrap');
    if(!wrap)return false;

    let fire=wrap.querySelector('.repo-horde-fire');
    if(!fire){
      fire=document.createElement('div');
      fire.className='repo-horde-fire';
      fire.setAttribute('aria-hidden','true');
      fire.innerHTML='<img alt=""><img alt="">';
      wrap.insertBefore(fire,wrap.firstChild);
    }

    const imgs=[...fire.querySelectorAll('img')];
    while(imgs.length<2){
      const img=document.createElement('img');img.alt='';fire.appendChild(img);imgs.push(img);
    }

    const paint=()=>{
      const src=`assets/nametags/wyrmfire-score-fire-${String(flameFrame).padStart(2,'0')}.png`;
      imgs.forEach((img,index)=>{
        if(img.getAttribute('src')!==src)img.src=src;
        img.style.transform=index?'scaleX(-1) translateY(2px)':'translateY(0)';
      });
      flameFrame=flameFrame%8+1;
    };
    paint();
  ...
  const style=document.createElement('style');
  style.id='repoPandaLowerAndHordeFlamesFinal';
  style.textContent=`
    #quidditchModePitch .pet-label[data-nametag="nametag_panda_rare"] .qm-custom-nametag>b{
    ...
    #endlessHordeSection .repo-horde-logo-wrap .repo-horde-fire{
      position:absolute!important;display:flex!important;visibility:visible!important;
      z-index:1!important;top:-15px!important;left:50%!important;width:460px!important;
      height:145px!important;transform:translateX(-50%)!important;justify-content:center!important;
      gap:0!important;opacity:.95!important;pointer-events:none!important;
    }
    #endlessHordeSection .repo-horde-logo-wrap .repo-horde-fire img{
      display:block!important;width:245px!important;height:145px!important;object-fit:contain!important;
      image-rendering:pixelated!important;filter:drop-shadow(0 0 10px #ff3b00)!important;
    }
    #endlessHordeSection .repo-horde-logo-wrap .repo-horde-logo-image{
      position:relative!important;z-index:2!important;
    }
    @media(max-width:700px){
      #endlessHordeSection .repo-horde-logo-wrap .repo-horde-fire{width:340px!important}
      #endlessHordeSection .repo-horde-logo-wrap .repo-horde-fire img{width:185px!important;height:125px!important}
    }
  `;
  document.head.appendChild(style);

  const schedule=()=>[0,50,160,420].forEach(delay=>setTimeout(restoreHordeFlames,delay));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});
  else schedule();
  document.addEventListener('click',event=>{
    if(event.target.closest('#endlessCombatMode,#standardCombatMode,[data-combat-menu],#combatButton,#openCombat'))schedule();
  });
})();

/* === PANDA FINAL TEXT CENTRE + THREE COMBAT WEAPONS ONLY === */
(()=>{
  const removedWeapons=new Set(['sword','bow','staff']);

  function applyFinalCombatWeaponOptions(){
    document.querySelectorAll('.combat-weapon-choice').forEach(button=>{
      if(removedWeapons.has(button.dataset.weapon))button.remove();
    });

    if(removedWeapons.has(selectedCombatWeapon)){
      selectedCombatWeapon='dharok';
      if(typeof selectCombatWeapon==='function')selectCombatWeapon('dharok');
    }

    document.querySelectorAll('.combat-weapon-choice').forEach(button=>{
      const selected=button.dataset.weapon===selectedCombatWeapon;
      button.classList.toggle('selected',selected);
      button.setAttribute('aria-pressed',selected?'true':'false');
    });
  }
  ...
  const style=document.createElement('style');
  style.id='repoPandaFinalCentreAndCombatWeapons';
  style.textContent=`
    #quidditchModePitch .pet-label[data-nametag="nametag_panda_rare"] .qm-custom-nametag>b,
    ...
    .combat-weapon-choice[data-weapon="sword"],
    .combat-weapon-choice[data-weapon="bow"],
    .combat-weapon-choice[data-weapon="staff"]{display:none!important}
    #combatDialog .combat-weapon-list,
    #combatDialog .endless-weapon-list{
      grid-template-columns:repeat(3,minmax(0,1fr))!important;
    }
  `;
  document.head.appendChild(style);

  const schedule=()=>[0,40,140,350].forEach(delay=>setTimeout(applyFinalCombatWeaponOptions,delay));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});
  else schedule();

  document.addEventListener('click',event=>{
    if(event.target.closest('[data-combat-menu],#combatButton,#openCombat,#standardCombatMode,#endlessCombatMode'))schedule();
  });
})();

/* === STANDARD SURVIVAL LOGO (MATCHES HORDE LOGO POSITION) === */
(()=>{
  function ensureStandardSurvivalLogo(){
    const dialog=document.getElementById('combatDialog');
    const switcher=document.getElementById('combatModeSwitcherSafe');
    if(!dialog||!switcher)return false;

    let wrap=document.getElementById('repoStandardModeLogoWrap');
    if(!wrap){
      wrap=document.createElement('div');
      wrap.id='repoStandardModeLogoWrap';
      wrap.className='repo-standard-logo-wrap';
      wrap.setAttribute('aria-hidden','true');
      wrap.innerHTML='<img class="repo-standard-logo-image" src="assets/combat/repo-standard-mode-logo.png" alt="Repo Standard Mode">';
    }

    if(wrap.parentElement!==dialog||switcher.nextElementSibling!==wrap){
      switcher.insertAdjacentElement('afterend',wrap);
    }

    const standardOpen=dialog.classList.contains('standard-menu-open')&&!dialog.classList.contains('combat-run-active');
    wrap.classList.toggle('is-visible',standardOpen);
    wrap.setAttribute('aria-hidden',standardOpen?'false':'true');
    return true;
  }

  const style=document.createElement('style');
  style.id='repoStandardModeLogoStyles';
  style.textContent=`
    #combatDialog .repo-standard-logo-wrap{
    ...
    #combatDialog.standard-menu-open:not(.combat-run-active) .repo-standard-logo-wrap.is-visible{
      display:flex!important;
      visibility:visible!important;
      opacity:1!important;
    }
    #combatDialog .repo-standard-logo-image{
      display:block;
      width:min(560px,72%);
      max-height:260px;
      object-fit:contain;
    ...
    @media(max-width:700px){
      #combatDialog .repo-standard-logo-wrap{min-height:110px;padding:4px 8px 10px}
      #combatDialog .repo-standard-logo-image{width:min(420px,88%);max-height:190px}
    }
  `;
  document.head.appendChild(style);

  const schedule=()=>[0,40,140,350].forEach(delay=>setTimeout(ensureStandardSurvivalLogo,delay));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});
  else schedule();

  document.addEventListener('click',event=>{
    if(event.target.closest('[data-combat-menu],#standardCombatMode,#endlessCombatMode,#combatButton,#openCombat,#combatStart,#endlessStartRun,#combatClose'))schedule();
  });

  const startObserver=()=>{
    const dialog=document.getElementById('combatDialog');
    if(!dialog)return setTimeout(startObserver,100);
    const observer=new MutationObserver(ensureStandardSurvivalLogo);
    observer.observe(dialog,{attributes:true,attributeFilter:['class'],childList:true});
    ensureStandardSurvivalLogo();
  };
  startObserver();
})();

/* === HORDE LEADERBOARD WEAPON + RARE DROP FINAL STYLES === */
(()=>{if(document.getElementById('repoRareDropHordeWeaponStyles'))return;const st=document.createElement('style');st.id='repoRareDropHordeWeaponStyles';st.textContent=`.endless-lb-row>span{min-width:0}.endless-lb-weapon{display:inline-block;margin-left:5px;padding:1px 4px;border:1px solid #594626;background:#171109;color:#b9aa8c;font-size:8px;line-height:1.2;vertical-align:1px;letter-spacing:.2px}`;document.head.appendChild(st)})();


// Tumeken's Shadow visual card icon — uses the supplied weapon asset without changing combat mechanics.
(function installTumekensShadowCardArt(){
  const apply=()=>{
    document.querySelectorAll('.combat-weapon-choice[data-weapon="shadow"]').forEach(button=>{
      if(button.querySelector('.tumekens-shadow-card-art')) return;
      const img=document.createElement('img');
      img.className='tumekens-shadow-card-art';
      img.src='assets/combat/tumekens-shadow-weapon.png';
      img.alt='';
  ...
  style.textContent=`
    .combat-weapon-choice[data-weapon="shadow"] .tumekens-shadow-card-art{
      width:28px;height:34px;object-fit:contain;vertical-align:middle;margin-right:6px;
      image-rendering:auto;filter:drop-shadow(0 1px 2px rgba(0,0,0,.7));
    }
  `;
  document.head.appendChild(style);
})();
...
// ============================================================
(function installQuidditchPenaltyEvent(){
  if(window.__repoQuidditchPenaltyInstalled)return;
  window.__repoQuidditchPenaltyInstalled=true;
  ...
  window.repoQuidditchPenaltyState=penalty;

  const later=(fn,ms)=>{
    const timer=setTimeout(()=>{penalty.timers.delete(timer);fn();},ms);
    penalty.timers.add(timer);
    return timer;
  };
  const clearTimers=()=>{for(const timer of penalty.timers)clearTimeout(timer);penalty.timers.clear();};
  ...
  style.textContent=`
    ...
    #quidditchModePitch .qm-penalty-caption{
      position:absolute;left:50%;top:69%;z-index:146;transform:translate(-50%,-50%);
      min-width:235px;max-width:76%;padding:8px 18px;border:2px solid #f3c44e;
      background:linear-gradient(#6b1c17,#3e0d0b);box-shadow:0 3px 0 #1c0706,0 0 16px rgba(255,194,54,.35);
      color:#ffe58c;font:900 15px/1.15 Georgia,serif;text-align:center;text-transform:uppercase;
      letter-spacing:.8px;pointer-events:none;animation:qmPenaltyCaption .18s ease-out both;
    }
    #quidditchModePitch .qm-penalty-caption.is-result{top:72%;font-size:17px}
    #quidditchModePitch .qm-penalty-spot{
      position:absolute;left:50%;top:51%;z-index:20;width:34px;height:34px;transform:translate(-50%,-50%);
      border:3px solid rgba(255,213,74,.92);border-radius:50%;box-shadow:0 0 0 4px rgba(90,15,12,.72),0 0 18px #ffc83d;
      pointer-events:none;animation:qmPenaltySpot .72s ease-in-out infinite alternate;
    }
    #quidditchModePitch.is-penalty-event .qm-pet{z-index:55!important}
    #quidditchModePitch.is-penalty-event .qm-pet.is-penalty-taker{z-index:70!important;filter:drop-shadow(0 0 8px #ffd63c)}
    #quidditchModePitch.is-penalty-event .qm-pet.is-penalty-keeper{z-index:68!important;filter:drop-shadow(0 0 7px rgba(255,255,255,.8))}
    @keyframes qmPenaltyPop{from{opacity:0;transform:translate(-50%,-50%) scale(.72)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    @keyframes qmPenaltyShake{0%,100%{transform:translate(-50%,-50%) rotate(0)}25%{transform:translate(calc(-50% - 5px),calc(-50% + 1px)) rotate(-1.5deg)}75%{transform:translate(calc(-50% + 5px),calc(-50% - 1px)) rotate(1.5deg)}}
  ...
  const playPenaltyWhistle=()=>{
    try{
      const whistle=new Audio('assets/quidditch-kickoff-whistle.mp3');
      whistle.preload='auto';whistle.volume=.92;whistle.currentTime=0;
      whistle.play().catch(()=>{});
      later(()=>{try{whistle.pause();whistle.src='';}catch(_){}},4200);
    }catch(_){}
  };

  const getPitch=()=>document.getElementById('quidditchModePitch');
  const removePenaltyNodes=()=>{
    penalty.overlay?.remove();penalty.overlay=null;
    penalty.caption?.remove();penalty.caption=null;
    penalty.ball?.remove();penalty.ball=null;
    const pitch=getPitch();
    pitch?.classList.remove('is-penalty-event');
    pitch?.querySelectorAll('.qm-penalty-alert,.qm-penalty-caption,.qm-penalty-spot').forEach(node=>node.remove());
    qmState?.pets?.forEach(p=>p.classList.remove('is-penalty-taker','is-penalty-keeper'));
  };

  const restoreBroadcast=()=>{
    window.qmCancelBarrySetPiece?.();
    removePenaltyNodes();
    penalty.active=false;penalty.test=false;
    if(!qmState?.open||qmState.liveState?.phase!=='live')return;
    qmState.busy=false;
    try{qmStartSyncedBroadcast(qmState.liveState);}catch(error){console.warn('Penalty broadcast resume:',error);}
  };

  const stopCurrentBroadcast=()=>{
    if(typeof qmSync!=='undefined'&&qmSync.raf){cancelAnimationFrame(qmSync.raf);qmSync.raf=null;}
    clearInterval(qmState.tick);qmState.tick=null;qmState.busy=true;qmState.carrier=null;
    const pitch=getPitch();
    pitch?.querySelectorAll('.qm-quaffle').forEach(node=>node.remove());
    qmState.pets.forEach(p=>p.classList.remove('has-ball'));
  };

  const setCaption=(text,result=false)=>{
    const pitch=getPitch();if(!pitch)return;
    penalty.caption?.remove();
    const caption=document.createElement('div');caption.className=`qm-penalty-caption${result?' is-result':''}`;caption.textContent=text;
    pitch.appendChild(caption);penalty.caption=caption;
  };

  const showPenaltyAlert=()=>{
    const pitch=getPitch();if(!pitch)return;
    const alert=document.createElement('div');alert.className='qm-penalty-alert';
    alert.innerHTML='<img src="assets/quidditch-penalty-popup.png" alt="Penalty">';
    pitch.appendChild(alert);penalty.overlay=alert;
  };

  const rosterForSide=(state,side)=>{
    const rows=(Array.isArray(state?.roster)?state.roster:[]).filter(row=>row.side===side);
    return rows;
  };
  const petForRow=(row,side)=>{
    const name=String(row?.pet_name||row?.petName||row?.username||'Pet').trim();
    return qmState.pets.find(p=>p.dataset.team===side&&p.dataset.name===name)||qmState.pets.find(p=>p.dataset.team===side)||null;
  };

  const gatherPlayers=(taker,keeper)=>{
    const pets=qmState.pets.filter(p=>p?.isConnected);
    const others=pets.filter(p=>p!==taker&&p!==keeper);
    const ring=[[-13,-13],[13,-13],[-16,11],[16,11],[-3,18],[3,-19],[20,0],[-20,0]];
    pets.forEach(p=>{p.style.transition='left 1.15s ease, top 1.15s ease, transform .25s ease';});
    others.forEach((p,index)=>{
      const [dx,dy]=ring[index%ring.length];qmMovePet(p,50+dx,51+dy,1.12);
    });
    if(taker){taker.classList.add('is-penalty-taker');qmMovePet(taker,50,51,1.05);}
    if(keeper){keeper.classList.add('is-penalty-keeper');qmMovePet(keeper,50+(keeper.dataset.team==='left'?-7:7),37,1.05);}
  };

  const commitPenaltyGoal=async(state,taker,side)=>{
    if(!state?.match_id||!taker)return null;
    const eventKey=`penalty:${state.match_id}`;
    try{
      const {data,error}=await db.rpc('record_live_quidditch_goal',{
        p_match_id:state.match_id,
        p_side:side,
        p_pet_name:String(taker.dataset.name||'Pet').slice(0,80),
        p_event_key:eventKey
      });
      if(error){console.warn('Quidditch penalty score sync:',error);return null;}
      const row=data?.[0]||null;if(row)qmApplyCanonicalScore(row);return row;
    }catch(error){console.warn('Quidditch penalty score sync:',error);return null;}
  };

  async function runPenaltyEvent({test=false,state=qmState.liveState}={}){
    if(penalty.active||!qmState?.open||state?.phase!=='live'||!qmState.pets?.length)return false;
    penalty.active=true;penalty.test=!!test;clearTimers();removePenaltyNodes();stopCurrentBroadcast();
    const pitch=getPitch();if(!pitch){penalty.active=false;return false;}
    pitch.classList.add('is-penalty-event');playPenaltyWhistle();showPenaltyAlert();window.qmStartBarrySetPiece?.('penalty');

    const matchId=String(state.match_id||'admin-test');
    let awardedSide=(qmHashNumber(`${matchId}:penalty-side`)%2===0)?'left':'right';
    if(!rosterForSide(state,awardedSide).length)awardedSide=awardedSide==='left'?'right':'left';
    const rows=rosterForSide(state,awardedSide);
    const row=rows[qmHashNumber(`${matchId}:penalty-taker`)%Math.max(1,rows.length)]||null;
    const taker=petForRow(row,awardedSide)||qmState.pets.find(p=>p.dataset.team===awardedSide)||qmState.pets[0];
    const defendingSide=awardedSide==='left'?'right':'left';
    const defenders=qmState.pets.filter(p=>p.dataset.team===defendingSide);
    const keeper=defenders[qmHashNumber(`${matchId}:penalty-keeper`)%Math.max(1,defenders.length)]||null;
    const takerName=taker?.dataset.name||'Pet';
    const teamName=awardedSide==='left'?qmState.leftName:qmState.rightName;
    const targetX=awardedSide==='left'?82.4:17.6,targetY=49.2;
    const scores=test?true:qmUnit(`${matchId}:penalty-score`)<.57;

    setCaption(`FOUL ON ${takerName} — PENALTY TO ${teamName}`);
    qmAddCommentary?.('penaltyAwarded',{pet:takerName,team:teamName},true);
    later(()=>gatherPlayers(taker,keeper),580);
    later(()=>{
      penalty.overlay?.remove();penalty.overlay=null;
      const spot=document.createElement('i');spot.className='qm-penalty-spot';pitch.appendChild(spot);
      if(keeper)qmMovePet(keeper,awardedSide==='left'?79.2:20.8,targetY,1.0);
      if(taker)qmMovePet(taker,50,51,.85);
      setCaption(`${takerName} STEPS UP…`);
    },1850);
    later(()=>{
      if(!penalty.active)return;
      const ball=document.createElement('i');ball.className='qm-quaffle';ball.style.left='50%';ball.style.top='51%';ball.style.zIndex='82';pitch.appendChild(ball);penalty.ball=ball;
      setCaption('PENALTY SHOT!');qmPlayRandomSound?.(qmShotSoundPaths);
      requestAnimationFrame(()=>requestAnimationFrame(()=>{
        ball.style.transition='left .62s ease-in, top .62s ease-in, transform .62s linear';
        ball.style.left=`${targetX}%`;ball.style.top=`${targetY}%`;ball.style.transform='translate(-50%,-50%) rotate(900deg) scale(.72)';
      }));
    },3150);
    later(async()=>{
      if(!penalty.active)return;
      if(scores){
        ...
        if(penalty.ball){
          penalty.ball.style.transition='left .22s ease-out, top .22s ease-out, transform .22s linear';
          penalty.ball.style.left=`${targetX+(awardedSide==='left'?-6:6)}%`;
          penalty.ball.style.top=`${targetY-8}%`;
          penalty.ball.style.transform='translate(-50%,-50%) rotate(1120deg) scale(.82)';
        }
        qmPlayRandomSound?.(qmReboundSoundPaths);setCaption('PENALTY MISSED!',true);
        qmAddCommentary?.('penaltyMissed',{pet:takerName,team:teamName},true);
        window.qmResolveBarrySetPiece?.(false,`${takerName} misses the penalty.`);
      }
    },3850);
    later(restoreBroadcast,5600);
    return true;
  }
  window.qmTestPenaltyEvent=()=>runPenaltyEvent({test:true,state:qmState.liveState});

  // Keep the deterministic broadcast paused while the set-piece animation plays.
  if(typeof qmStartSyncedBroadcast==='function'){
    const baseStartSyncedBroadcast=qmStartSyncedBroadcast;
    qmStartSyncedBroadcast=function(state){if(penalty.active)return;return baseStartSyncedBroadcast(state);};
  }
  if(typeof qmShowSharedGoal==='function'){
    const baseShowSharedGoal=qmShowSharedGoal;
    qmShowSharedGoal=function(state){if(penalty.active)return;return baseShowSharedGoal(state);};
  }
  if(typeof qmApplyLiveState==='function'){
    const baseApplyLiveState=qmApplyLiveState;
    qmApplyLiveState=function(state){
      const result=baseApplyLiveState(state);
      if(!state||!qmState.open)return result;
      const matchId=String(state.match_id||'');
      if(matchId!==penalty.matchId){
        penalty.matchId=matchId;penalty.autoTriggered=false;
        if(penalty.active&&!penalty.test){clearTimers();restoreBroadcast();}
      }
      if(penalty.active){
        if(typeof qmSync!=='undefined'&&qmSync.raf){cancelAnimationFrame(qmSync.raf);qmSync.raf=null;}
        return result;
  ...
  const ensureAdminButton=()=>{
    const panel=document.getElementById('qmAdminSpecialTester');
    if(!panel||document.getElementById('qmTestPenaltyEvent'))return;
    const button=document.createElement('button');button.type='button';button.id='qmTestPenaltyEvent';button.textContent='TEST PENALTY';
    button.title='Preview a Quidditch penalty without changing the score';
    button.addEventListener('click',()=>{
      const isCat=String(character?.username||'').toLowerCase()==='catasthma';
      if(!isCat||!toaState?.adminMode)return;
      if(!qmState.open||qmState.liveState?.phase!=='live'){toast('Open Quidditch Mode during a live match to test a penalty.',3600);return;}
      runPenaltyEvent({test:true,state:qmState.liveState});
    });
    const allButton=panel.querySelector('#qmTestAllSpecials');
  ...
  const baseClose=closeQuidditchMode;
  closeQuidditchMode=function(){abortPenalty();return baseClose();};
  document.getElementById('quidditchModeClose')?.addEventListener('click',abortPenalty,true);
  document.getElementById('quidditchModeOverlay')?.addEventListener('click',event=>{if(event.target===event.currentTarget)abortPenalty();},true);
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&penalty.active)abortPenalty();},true);
})();
...
// ============================================================
(function installQuidditchGoldenSnitchEvent(){
  if(window.__repoQuidditchSnitchInstalled)return;
  window.__repoQuidditchSnitchInstalled=true;
  ...
  window.repoQuidditchSnitchState=snitch;

  const later=(fn,ms)=>{
    const timer=setTimeout(()=>{snitch.timers.delete(timer);fn();},ms);
    snitch.timers.add(timer);
    return timer;
  };
  ...
  style.textContent=`
    #quidditchModePitch .qm-snitch-alert{
      position:absolute;left:50%;top:47%;z-index:150;width:min(430px,52%);
      transform:translate(-50%,-50%);pointer-events:none;
      filter:drop-shadow(0 10px 14px rgba(0,0,0,.82));
      animation:qmSnitchPop .22s ease-out both,qmSnitchShake .13s linear .22s 7;
    }
    #quidditchModePitch .qm-snitch-alert img{display:block;width:100%;height:auto;image-rendering:pixelated}
    #quidditchModePitch .qm-snitch-caption{
      position:absolute;left:50%;top:70%;z-index:151;transform:translate(-50%,-50%);
      min-width:265px;max-width:82%;padding:8px 18px;border:2px solid #f3c44e;
      background:linear-gradient(#6b1c17,#3e0d0b);box-shadow:0 3px 0 #1c0706,0 0 16px rgba(255,194,54,.35);
      color:#ffe58c;font:900 15px/1.15 Georgia,serif;text-align:center;text-transform:uppercase;
      letter-spacing:.8px;pointer-events:none;animation:qmSnitchCaption .18s ease-out both;
    }
    #quidditchModePitch .qm-snitch-caption.is-result{top:74%;font-size:17px}
    #quidditchModePitch.is-snitch-event .qm-pet{z-index:58!important}
    #quidditchModePitch.is-snitch-event .qm-pet.is-snitch-catcher{z-index:78!important;filter:drop-shadow(0 0 5px #fff) drop-shadow(0 0 14px #ffcf35);animation:qmSnitchCatchPulse .34s ease-in-out 3 alternate}
    #quidditchModePitch .qm-snitch-catcher-label{position:absolute;left:50%;bottom:calc(100% + 8px);transform:translateX(-50%);white-space:nowrap;padding:5px 9px;border:2px solid #ffd84d;border-radius:8px;background:linear-gradient(#67170f,#310906);box-shadow:0 3px 0 #160403,0 0 12px rgba(255,215,65,.48);color:#fff0a6;font:900 12px/1 Georgia,serif;letter-spacing:.55px;text-transform:uppercase;pointer-events:none;animation:qmSnitchCaughtLabel .18s ease-out both}
    @keyframes qmSnitchCatchPulse{from{transform:scale(1)}to{transform:scale(1.09)}}
    @keyframes qmSnitchCaughtLabel{from{opacity:0;transform:translate(-50%,6px) scale(.9)}to{opacity:1;transform:translate(-50%,0) scale(1)}}
    #quidditchModePitch .qm-golden-snitch{
      position:absolute;left:50%;top:50%;z-index:90;width:40px;height:24px;
      transform:translate(-50%,-50%);pointer-events:none;
      filter:drop-shadow(0 0 5px #fff3a0) drop-shadow(0 0 9px rgba(255,201,44,.9));
      animation:qmSnitchBob .34s ease-in-out infinite alternate;
    }
    #quidditchModePitch .qm-golden-snitch img{
      display:block;width:100%;height:100%;object-fit:contain;image-rendering:pixelated;
    }
    #quidditchModePitch .qm-pet.is-snitch-bumped>.pet-sprite,
    #quidditchModePitch .qm-pet.is-snitch-bumped>.qm-broom{
      animation:qmSnitchBump .42s ease-out both;
    }
    #quidditchModePitch .qm-snitch-winner{
      position:absolute;left:50%;top:16%;z-index:152;transform:translateX(-50%);
      min-width:290px;max-width:84%;padding:10px 16px 12px;border:2px solid #ffd24a;border-radius:12px;
      background:linear-gradient(#54120e,#240706);box-shadow:0 8px 18px rgba(0,0,0,.6),0 0 16px rgba(255,214,74,.28);
      text-align:center;pointer-events:none;animation:qmSnitchWinner .22s ease-out both;
    }
    #quidditchModePitch .qm-snitch-winner strong{display:block;color:#ffeaa0;font:900 14px/1.05 Georgia,serif;letter-spacing:.95px;text-transform:uppercase}
    #quidditchModePitch .qm-snitch-winner b{display:block;margin-top:4px;color:#fff3c8;font:900 22px/1 Georgia,serif;text-transform:uppercase}
    #quidditchModePitch .qm-snitch-winner small{display:block;margin-top:5px;color:#f2d992;font:700 11px/1.15 Georgia,serif;letter-spacing:.35px;text-transform:uppercase}
    @keyframes qmSnitchPop{from{opacity:0;transform:translate(-50%,-50%) scale(.72)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    @keyframes qmSnitchShake{0%,100%{transform:translate(-50%,-50%) rotate(0)}25%{transform:translate(calc(-50% - 5px),calc(-50% + 1px)) rotate(-1.5deg)}75%{transform:translate(calc(-50% + 5px),calc(-50% - 1px)) rotate(1.5deg)}}
    @keyframes qmSnitchCaption{from{opacity:0;transform:translate(-50%,-42%) scale(.9)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    ...
    @media(max-width:760px){
      ...
      #quidditchModePitch .qm-golden-snitch{width:32px;height:19px}
      #quidditchModePitch .qm-snitch-winner{min-width:210px;padding:8px 10px 9px}
      #quidditchModePitch .qm-snitch-winner b{font-size:16px}
    }
  `;
  document.head.appendChild(style);

  const playSnitchAppearSound=()=>{
    try{
      const audio=new Audio('assets/quidditch-snitch-appear.mp3');
      audio.preload='auto';audio.volume=.82;audio.currentTime=0;
      audio.play().catch(()=>{});
      later(()=>{try{audio.pause();audio.src='';}catch(_){}},6200);
    }catch(_){}
  };

  let snitchEventAudio=null;
  const stopSnitchEventAudio=()=>{
    if(!snitchEventAudio)return;
    try{snitchEventAudio.pause();snitchEventAudio.currentTime=0;snitchEventAudio.src='';}catch(_){}
    snitchEventAudio=null;
  };
  const startSnitchEventAudio=()=>{
    stopSnitchEventAudio();
    try{
      const audio=new Audio('assets/quidditch-snitch-event-voice.mp3');
      audio.preload='auto';audio.volume=.13;audio.currentTime=0;
      snitchEventAudio=audio;
      audio.play().catch(()=>{});
    }catch(_){}
  };

  const removeSnitchNodes=()=>{
    stopSnitchEventAudio();
    snitch.alert?.remove();snitch.alert=null;
    snitch.caption?.remove();snitch.caption=null;
    snitch.sprite?.remove();snitch.sprite=null;
    snitch.winnerBanner?.remove();snitch.winnerBanner=null;
    const pitch=getPitch();
  ...
  const setCaption=(text,result=false)=>{
    const pitch=getPitch();if(!pitch)return;
    snitch.caption?.remove();
    const caption=document.createElement('div');
    caption.className=`qm-snitch-caption${result?' is-result':''}`;
    caption.textContent=text;
    pitch.appendChild(caption);snitch.caption=caption;
  };

  const showAlert=()=>{
    const pitch=getPitch();if(!pitch)return;
    const alert=document.createElement('div');alert.className='qm-snitch-alert';
    alert.innerHTML='<img src="assets/quidditch-snitch-popup.png" alt="Golden Snitch has appeared">';
    pitch.appendChild(alert);snitch.alert=alert;
  };

  const createSnitchSprite=()=>{
    const pitch=getPitch();if(!pitch)return null;
    snitch.sprite?.remove();
    const el=document.createElement('div');
    el.className='qm-golden-snitch';
    el.innerHTML='<img src="assets/quidditch-golden-snitch.png" alt="Golden Snitch">';
    el.dataset.x='50';el.dataset.y='50';
    pitch.appendChild(el);snitch.sprite=el;
    return el;
  };

  const setSnitchPosition=(x,y)=>{
    const el=snitch.sprite||createSnitchSprite();if(!el)return;
    const nx=Math.max(8,Math.min(92,Number(x)||50));
    const ny=Math.max(14,Math.min(86,Number(y)||50));
    el.dataset.x=String(nx);el.dataset.y=String(ny);
    el.style.transition='none';el.style.left=`${nx}%`;el.style.top=`${ny}%`;
  };

  const seededChasePoints=matchId=>{
    const count=8+(qmHashNumber(`${matchId}:snitch-path-count`)%4); // 8–11 genuinely different turns
    const edge=qmHashNumber(`${matchId}:snitch-entry-edge`)%4;
    const edgePos=.18+qmUnit(`${matchId}:snitch-entry-pos`)*.64;
    const first=edge===0?{x:8,y:18+edgePos*64}:edge===1?{x:92,y:18+edgePos*64}:edge===2?{x:12+edgePos*76,y:15}:{x:12+edgePos*76,y:85};
    const points=[first];
    let previous=first;
    for(let i=1;i<count;i++){
      let chosen=null;
      for(let attempt=0;attempt<9;attempt++){
        const x=11+qmUnit(`${matchId}:snitch-path-x:${i}:${attempt}`)*78;
        const y=17+qmUnit(`${matchId}:snitch-path-y:${i}:${attempt}`)*66;
        const distance=Math.hypot(x-previous.x,y-previous.y);
        if(distance>=23||attempt===8){chosen={x,y};break;}
      }
      points.push({x:Number(chosen.x.toFixed(3)),y:Number(chosen.y.toFixed(3))});
      previous=chosen;
    }
    return points;
  };
  ...
  const sampleChasePath=(points,progress)=>{
    if(!points?.length)return{x:50,y:50};
    if(points.length===1)return points[0];
    const scaled=Math.max(0,Math.min(.999999,progress))*(points.length-1);
    const index=Math.floor(scaled),t=scaled-index;
    ...
    return{x:Math.max(8,Math.min(92,catmull(p0.x,p1.x,p2.x,p3.x,t))),y:Math.max(14,Math.min(86,catmull(p0.y,p1.y,p2.y,p3.y,t)))};
  };
  const smoothStep=value=>{const t=Math.max(0,Math.min(1,value));return t*t*(3-2*t);};

  const bumpPair=(pets,mark,seed=snitch.matchId)=>{
    if(pets.length<2)return;
    const first=qmHashNumber(`${seed}:bump-a:${mark}`)%pets.length;
    let second=qmHashNumber(`${seed}:bump-b:${mark}`)%pets.length;
    if(second===first)second=(second+1)%pets.length;
    [pets[first],pets[second]].forEach((pet,index)=>{
      if(!pet)return;
      pet.style.setProperty('--snitch-bump-x',index===0?'8px':'-8px');
      pet.style.setProperty('--snitch-bump-r',index===0?'13deg':'-13deg');
      pet.classList.remove('is-snitch-bumped');void pet.offsetWidth;pet.classList.add('is-snitch-bumped');
      later(()=>pet.classList.remove('is-snitch-bumped'),460);
    });
  };

  const startSmoothChase=(matchId,durationMs,onComplete)=>{
    stopChaseAnimation();
    const pets=(qmState.pets||[]).filter(p=>p?.isConnected);
    const points=seededChasePoints(matchId);
    const starts=pets.map(p=>({x:Number(p.dataset.qx||50),y:Number(p.dataset.qy||50)}));
    const marks=new Set();
    snitch.chasePath=points;snitch.chaseStartedAt=performance.now();snitch.chaseDuration=durationMs;
    const first=points[0]||{x:50,y:50};setSnitchPosition(first.x,first.y);

    const frame=now=>{
      if(!snitch.active){snitch.raf=null;return;}
      const progress=Math.max(0,Math.min(1,(now-snitch.chaseStartedAt)/Math.max(1,durationMs)));
      const snitchPoint=sampleChasePath(points,progress);
      setSnitchPosition(snitchPoint.x,snitchPoint.y);

      pets.forEach((pet,index)=>{
        const lag=.045+qmUnit(`${matchId}:snitch-lag:${index}`)*.105;
        const followProgress=Math.max(0,Math.min(1,(progress-lag)/(1-lag)));
        const pathPoint=sampleChasePath(points,followProgress);
        const angle=(index/Math.max(1,pets.length))*Math.PI*2+Math.sin(progress*Math.PI*5+index*1.7)*.34;
        const radius=3.2+(index%4)*1.05+qmUnit(`${matchId}:snitch-radius:${index}`)*1.8;
        let tx=pathPoint.x+Math.cos(angle)*radius;
        let ty=pathPoint.y+Math.sin(angle)*radius*.72;
        if(progress<.16){
          const blend=smoothStep(progress/.16);
          tx=starts[index].x+(tx-starts[index].x)*blend;
          ty=starts[index].y+(ty-starts[index].y)*blend;
        }
        tx=Math.max(6,Math.min(94,tx));ty=Math.max(15,Math.min(85,ty));
        const oldX=Number(pet.dataset.qx||tx);
        pet.dataset.qx=String(tx);pet.dataset.qy=String(ty);
        pet.style.transition='none';pet.style.left=`${tx}%`;pet.style.top=`${ty}%`;
        pet.classList.toggle('is-leftward',tx<oldX-.025);
      });

      if(progress>=.30&&!marks.has('bump1')){marks.add('bump1');bumpPair(pets,'one',matchId);setCaption('PLAYERS COLLIDE IN THE CHASE!');}
      if(progress>=.58&&!marks.has('call')){marks.add('call');setCaption('THE WHOLE PITCH CHASES THE SNITCH…');}
      if(progress>=.76&&!marks.has('bump2')){marks.add('bump2');bumpPair(pets,'two',matchId);}

      if(progress<1)snitch.raf=requestAnimationFrame(frame);
      else{snitch.raf=null;onComplete?.();}
    };
    snitch.raf=requestAnimationFrame(frame);
  };

  const distanceToSnitch=pet=>{
    const sx=Number(snitch.sprite?.dataset.x||50),sy=Number(snitch.sprite?.dataset.y||50);
    const px=Number(pet?.dataset.qx||50),py=Number(pet?.dataset.qy||50);
    return Math.hypot(sx-px,sy-py);
  };

  const chooseCatcher=matchId=>{
    const pets=(qmState.pets||[]).filter(p=>p?.isConnected);
    if(!pets.length)return null;
    return pets.slice().sort((a,b)=>distanceToSnitch(a)-distanceToSnitch(b)||
      (qmHashNumber(`${matchId}:catcher:${a.dataset.name}`)-qmHashNumber(`${matchId}:catcher:${b.dataset.name}`)))[0]||pets[0];
  };

  const showWinnerBanner=(teamName,petName)=>{
    const pitch=getPitch();if(!pitch)return;
    snitch.winnerBanner?.remove();
    const banner=document.createElement('div');banner.className='qm-snitch-winner';
    banner.innerHTML=`<strong>GOLDEN SNITCH CAUGHT BY ${escapeHtml(String(petName||'Pet').toUpperCase())}!</strong><b>${escapeHtml(String(teamName||'TEAM').toUpperCase())} WIN</b><small>THE MATCH ENDS IMMEDIATELY</small>`;
    pitch.appendChild(banner);snitch.winnerBanner=banner;
  };

  const ensureWinningScores=(state,winnerSide,canonical=null)=>{
    const next={...(state||{})};
    const source=canonical||{};
    let left=Number(source.left_score??next.left_score)||0;
    let right=Number(source.right_score??next.right_score)||0;
    if(winnerSide==='left'&&left<=right)left=right+1;
    if(winnerSide==='right'&&right<=left)right=left+1;
    next.left_score=left;next.right_score=right;
    next.left_scorers=source.left_scorers||next.left_scorers||{};
    next.right_scorers=source.right_scorers||next.right_scorers||{};
    next.snitch_winner_side=winnerSide;
    next.snitch_winner_pet=snitch.winnerPet;
    return next;
  };
  ...
  const commitSnitchWin=async(state,catcher,winnerSide)=>{
    ...
    const goalCount=Math.min(30,required);
    let canonical=null;
  ...
  const applyLockedPresentation=state=>{
    ...
    qmState.leftName=String(state.left_name||qmState.leftName||'LEFT');
    qmState.rightName=String(state.right_name||qmState.rightName||'RIGHT');
    qmApplyCanonicalScore({
      left_score:state.left_score,right_score:state.right_score,
      left_scorers:state.left_scorers||{},right_scorers:state.right_scorers||{}
    });
    if($('qmStatus'))$('qmStatus').textContent='GOLDEN SNITCH';
    if($('qmTimer'))$('qmTimer').textContent='FINAL';
  };

  const restoreBroadcast=()=>{
    window.qmStopBarrySnitchFlybys?.();
    window.qmCancelBarrySetPiece?.();
    removeSnitchNodes();snitch.active=false;snitch.test=false;
    if(snitch.lockedFinal||!qmState?.open||qmState.liveState?.phase!=='live')return;
    qmState.busy=false;
    try{qmStartSyncedBroadcast(qmState.liveState);}catch(error){console.warn('Snitch broadcast resume:',error);}
  };

  async function runSnitchEvent({test=false,state=qmState.liveState}={}){
    if(snitch.active||snitch.lockedFinal||!qmState?.open||state?.phase!=='live'||!qmState.pets?.length)return false;
    if(window.repoQuidditchPenaltyState?.active)return false;
    snitch.active=true;snitch.test=!!test;snitch.latestState=state;
    clearTimers();removeSnitchNodes();stopCurrentBroadcast();
    const pitch=getPitch();if(!pitch){snitch.active=false;return false;}
    pitch.classList.add('is-snitch-event');
    snitch.matchId=getMatchId(state);
    const eventSeed=test?`${snitch.matchId}:admin-preview:${Date.now()}`:snitch.matchId;
    playSnitchAppearSound();startSnitchEventAudio();showAlert();setCaption('GOLDEN SNITCH HAS APPEARED!');
    window.qmStartBarrySetPiece?.('snitch');
    qmAddCommentary?.('snitchAppeared',{},true);

    const chaseMs=5000+((qmHashNumber(`${eventSeed}:snitch-duration`)%6)*1000); // 5–10 seconds
    window.qmStartBarrySnitchFlybys?.(chaseMs);

    const finishCatch=async()=>{
      if(!snitch.active)return;
      stopSnitchEventAudio();
      const catcher=chooseCatcher(eventSeed)||qmState.pets[0];
      const sx=Number(snitch.sprite?.dataset.x||50),sy=Number(snitch.sprite?.dataset.y||50);
      if(catcher){
        catcher.classList.add('is-snitch-catcher','is-celebrating');
        catcher.style.transition='left .34s ease-out,top .34s ease-out,transform .25s ease';
        catcher.dataset.qx=String(sx);catcher.dataset.qy=String(sy);
        catcher.style.left=`${sx}%`;catcher.style.top=`${sy}%`;
        catcher.querySelector('.qm-snitch-catcher-label')?.remove();
        const catcherLabel=document.createElement('div');
      ...
      window.qmStopBarrySnitchFlybys?.();
      window.qmResolveBarrySetPiece?.(true,`${petName} catches the Golden Snitch!`);
      qmPlayGoalSound?.();qmFireworks?.(sx,sy);
    ...
    later(()=>{
      ...
      snitch.alert?.remove();snitch.alert=null;
      createSnitchSprite();
      setCaption('ALL PLAYERS ABANDON THE QUAFFLE!');
      startSmoothChase(eventSeed,chaseMs,finishCatch);
    },1280);
    return true;
  }
  window.qmTestSnitchEvent=()=>runSnitchEvent({test:true,state:qmState.liveState});

  // Keep all normal Quaffle actions paused during the chase and after the catch.
  if(typeof qmStartSyncedBroadcast==='function'){
    const baseStart=qmStartSyncedBroadcast;
    qmStartSyncedBroadcast=function(state){if(snitch.active||snitch.lockedFinal)return;return baseStart(state);};
  }
  if(typeof qmShowSharedGoal==='function'){
    const baseGoal=qmShowSharedGoal;
    qmShowSharedGoal=function(state){if(snitch.active||snitch.lockedFinal)return;return baseGoal(state);};
  }

  if(typeof qmApplyLiveState==='function'){
    const baseApply=qmApplyLiveState;
    qmApplyLiveState=function(state){
      if(!state)return baseApply(state);
      const incomingId=String(state.match_id||'');
      const currentId=String(snitch.matchId||'');
      ...
      // No narrow timing window: viewers joining after the trigger still receive
      // the same shared event rather than seeing the supposedly finished match.
      if(elapsed>=triggerAt){
        snitch.autoTriggered=true;runSnitchEvent({test:false,state});
      }
      return result;
    };
  }
  ...
  const ensureAdminButton=()=>{
    const panel=document.getElementById('qmAdminSpecialTester');
    if(!panel||document.getElementById('qmTestSnitchEvent'))return;
    const button=document.createElement('button');
    button.type='button';button.id='qmTestSnitchEvent';button.textContent='TEST SNITCH';
    button.title='Preview the Golden Snitch chase without changing the score or ending the match';
    button.addEventListener('click',()=>{
      const isCat=String(character?.username||'').toLowerCase()==='catasthma';
      if(!isCat||!toaState?.adminMode)return;
      if(!qmState.open||qmState.liveState?.phase!=='live'){
        toast('Open Quidditch Mode during a live match to test the Golden Snitch.',3600);return;
      }
      runSnitchEvent({test:true,state:qmState.liveState});
    });
    const penaltyButton=panel.querySelector('#qmTestPenaltyEvent');
    if(penaltyButton?.parentElement)penaltyButton.parentElement.appendChild(button);
    else panel.appendChild(button);
  };
  ...
  window.qmAbortSnitchEvent=abortSnitch;
  const baseClose=closeQuidditchMode;
  closeQuidditchMode=function(){abortSnitch();return baseClose();};
...
(() => {
  ...
  function updateRepoHordeBuildBadge() {
    ...
    h.picks.forEach(pick=>{
      const key=String(pick.id||pick.name||'upgrade');
      let item=groupedById.get(key);
      if(!item){item={name:pick.name,icon:pick.icon,count:0,rare:false};groupedById.set(key,item);grouped.push(item);}
      item.count+=1;item.rare=item.rare||Boolean(pick.rare);
    });
    const allUpgrades=grouped.map(p=>`<span class="${p.rare?'golden':''}">${p.icon} ${p.name}${p.count>1?` ×${p.count}`:''}</span>`).join('');
    badge.innerHTML=`<b>HORDE BUILD</b><small>${h.picks.length} upgrades${h.rarePicks?` • ${h.rarePicks} GOLDEN`:''}</small><div>${allUpgrades||'<span>No upgrades yet</span>'}</div>`;
  }
  ...
  killCombatEnemy=function(enemy){
    ...
    if(!h.processingExplosion&&h.graveburstChance>0&&Math.random()<h.graveburstChance){
      ...
      s.repoHordeExplosions=s.repoHordeExplosions||[];
      s.repoHordeExplosions.push({x,y,life:.62,maxLife:.62,seed:Math.random()*1000});
  ...
  hordeUpgradeStyle.textContent=`
    ...
    .repo-horde-build-badge{display:grid;grid-template-columns:auto auto 1fr;align-items:center;gap:7px 10px;margin:8px 0;padding:9px 11px 10px;border:1px solid #6c582e;background:#0d120be8;color:#ded1aa;font-size:9px;line-height:1.15;min-height:44px;box-sizing:border-box}
    .repo-horde-build-badge.hidden{display:none!important}.repo-horde-build-badge>b{color:#b6df91;letter-spacing:1px}.repo-horde-build-badge>small{color:#9e9684}.repo-horde-build-badge>div{grid-column:1/-1;display:flex;gap:5px;flex-wrap:wrap;width:100%;margin:1px 0 0;align-content:flex-start}.repo-horde-build-badge span{padding:4px 6px;border:1px solid #4d5b40;background:#1a2117;white-space:nowrap}.repo-horde-build-badge span.golden{border-color:#d6a935;background:#5a3e0d;color:#ffe68b;box-shadow:0 0 7px #e8ba3e55}
...
/* === ENDLESS HORDE RUN POLISH: GRAVEBURST FX, RUN LOGO + LEADERBOARD HOVERCARDS === */
(()=>{
  const hordeActive=()=>Boolean(combatRunning&&combatState?.zombie);

  const updateBeforeHordeExplosion=updateCombat;
  updateCombat=function(dt,now){
    const result=updateBeforeHordeExplosion.apply(this,arguments);
    const explosions=combatState?.repoHordeExplosions;
    if(Array.isArray(explosions)){
      explosions.forEach(fx=>fx.life-=dt);
      combatState.repoHordeExplosions=explosions.filter(fx=>fx.life>0);
    }
    return result;
  };

  const drawBeforeHordeExplosion=drawCombat;
  drawCombat=function(){
    const result=drawBeforeHordeExplosion.apply(this,arguments);
    const s=combatState,explosions=s?.repoHordeExplosions;
    if(!s?.zombie||!Array.isArray(explosions)||!explosions.length)return result;
    const canvas=document.getElementById('combatCanvas'),ctx=canvas?.getContext('2d');
    if(!ctx)return result;
    explosions.forEach(fx=>{
      const progress=1-Math.max(0,fx.life)/Math.max(.01,fx.maxLife||.62);
      const fade=Math.max(0,1-progress);
      const radius=7+progress*36;
      ctx.save();ctx.translate(fx.x,fx.y);
      const glow=ctx.createRadialGradient(0,0,0,0,0,radius);
      glow.addColorStop(0,`rgba(255,255,220,${.95*fade})`);
      glow.addColorStop(.2,`rgba(255,214,69,${.9*fade})`);
      glow.addColorStop(.55,`rgba(255,91,20,${.62*fade})`);
      glow.addColorStop(1,'rgba(55,8,4,0)');
      ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,0,radius,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle=`rgba(255,225,112,${.9*fade})`;ctx.lineWidth=Math.max(1,4*(1-progress));ctx.beginPath();ctx.arc(0,0,9+progress*28,0,Math.PI*2);ctx.stroke();
      for(let i=0;i<12;i++){
        const angle=(Math.PI*2*i/12)+(fx.seed||0);
        const distance=5+progress*(24+(i%4)*5);
        const size=Math.max(1,4-progress*2.5);
        ctx.save();ctx.rotate(angle);ctx.translate(distance,0);ctx.rotate(progress*5+i);
        ctx.fillStyle=i%3===0?`rgba(255,248,180,${fade})`:i%3===1?`rgba(255,169,40,${fade})`:`rgba(205,55,20,${fade})`;
        ctx.fillRect(-size/2,-size/2,size,size);ctx.restore();
      }
      if(progress>.38){
        const smoke=(progress-.38)/.62;
        for(let i=0;i<4;i++){
          ctx.fillStyle=`rgba(45,39,34,${(1-smoke)*.34})`;
          ctx.beginPath();ctx.arc((i-1.5)*6,(smoke*18)-8-(i%2)*4,5+smoke*5,0,Math.PI*2);ctx.fill();
        }
      }
      ctx.restore();
    });
  ...
  style.textContent=`
    ...
    #repoHordeRunHoverCard.show{opacity:1;visibility:visible;pointer-events:auto;transform:none}

    #repoHordeRunHoverCard::backdrop{background:transparent;pointer-events:none}
    .repo-horde-run-identity{position:relative;display:flex;align-items:flex-end;gap:10px;min-height:92px;margin:-3px -3px 10px;padding:10px;overflow:hidden;border:1px solid #6d5128;background:linear-gradient(135deg,#312319,#120d09)}
    .repo-horde-run-identity.has-backdrop{background-image:linear-gradient(180deg,#0002,#000b),var(--repo-run-backdrop);background-size:cover;background-position:center}
    .repo-horde-run-identity-shade{position:absolute;inset:0;background:linear-gradient(90deg,#09070522,#09070588 56%,#090705dd);pointer-events:none}
    .repo-horde-run-identity>img{position:relative;z-index:1;width:72px;height:82px;object-fit:contain;object-position:center bottom;filter:drop-shadow(0 3px 3px #000);image-rendering:pixelated}
    .repo-horde-run-identity>div:last-child{position:relative;z-index:1;min-width:0;padding-bottom:4px}.repo-horde-run-identity small{display:block;color:#d7b96d;font-size:8px;font-weight:900;letter-spacing:1.2px}.repo-horde-run-identity b{display:block;color:#fff0c2;font:900 17px Georgia,serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.repo-horde-run-identity span{display:block;margin-top:2px;color:#c7b995;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    #repoHordeRunHoverCard header{display:flex;align-items:center;justify-content:space-between;gap:10px;padding-bottom:8px;border-bottom:1px solid #594425}
    #repoHordeRunHoverCard header small{display:block;color:#9f8e70;font-size:8px;letter-spacing:1.2px}#repoHordeRunHoverCard header b{display:block;color:#f1d48b;font:900 15px Georgia,serif}#repoHordeRunHoverCard header>strong{color:#ffe28b;font:900 23px Georgia,serif;text-shadow:0 0 8px #e6ae3f66}
    .repo-horde-run-hover-stats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin:9px 0}.repo-horde-run-hover-stats span{min-width:0;padding:6px;border:1px solid #44351f;background:#0a0806}.repo-horde-run-hover-stats small{display:block;color:#8e8069;font-size:8px;letter-spacing:.7px}.repo-horde-run-hover-stats b{display:block;margin-top:2px;color:#eee1bd;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .repo-horde-run-hover-upgrades>small{display:block;margin-bottom:5px;color:#b7d596;font-size:8px;font-weight:900;letter-spacing:1px}.repo-horde-run-hover-upgrades>div{display:flex;flex-wrap:wrap;gap:4px;max-height:116px;overflow-y:auto;padding-right:2px}.repo-horde-run-hover-upgrades span{padding:3px 5px;border:1px solid #4c593e;background:#182016;color:#d5dfc5;font-size:9px;white-space:nowrap}.repo-horde-run-hover-upgrades span.golden{border-color:#c99c34;background:#4b350d;color:#ffe494;box-shadow:0 0 6px #daa83844}.repo-horde-run-hover-upgrades span i{color:#fff0a4;font-size:8px;font-style:normal}.repo-horde-run-hover-upgrades em{color:#8f8577;font-size:10px}
    @media(max-width:700px){#repoCombatSurvivalRunLogo{height:68px;padding:0 26px}#repoCombatSurvivalRunLogo img{max-height:64px;width:min(360px,82%)}}
  `;
  document.head.appendChild(style);

  const scheduleLogo=()=>[0,50,150,350].forEach(delay=>setTimeout(syncRepoCombatRunLogo,delay));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',scheduleLogo,{once:true});else scheduleLogo();
