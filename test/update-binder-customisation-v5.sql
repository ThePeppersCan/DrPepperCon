-- REPO COMPANY: BINDER CUSTOMISATION V5
-- Run this whole file once in Supabase -> SQL Editor.
-- Extends Inferno/Celestial across the complete 18-pocket spread and adds six natural
-- permanent 10,000 GP legendary animations. Existing purchases remain owned.
...
set binder_legendary_effects = coalesce((
  ...
  where lower(btrim(effect_name)) in (
    ...
    'stormwind','smokeveil','autumnstream','tidalstream','dawnmist','nightrain'
  )
), '{}'::text[]);
...
-- V5-compatible response using the existing V3 RPC name so deployed sites keep working.
