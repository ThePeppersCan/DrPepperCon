...
create or replace function public.claim_missing_repoggle_rewards()
returns table(
  levels_repaired integer,
  gold_awarded bigint,
  xp_awarded bigint,
  new_gp bigint,
  new_runecrafting_xp bigint
...
begin
  ...
  -- Lock the pending rows before calculating and awarding their fixed,
  -- server-owned rewards. Concurrent calls therefore cannot double claim.
  with pending as (
    select p.level_number,c.gold_reward,c.xp_reward
      from public.repoggle_progress p
      join public.repoggle_level_config c using(level_number)
     where p.user_id=v_user and not p.reward_claimed
     for update of p
  )
  select count(*)::integer,coalesce(sum(gold_reward),0)::bigint,coalesce(sum(xp_reward),0)::bigint
    into v_levels,v_gold,v_xp
    from pending;

  if v_levels>0 then
    update public.characters
       set gp=coalesce(gp,0)+v_gold,
           runecrafting_xp=coalesce(runecrafting_xp,0)+v_xp
     where user_id=v_user
     returning gp,runecrafting_xp into v_new_gp,v_new_xp;
    if not found then raise exception 'Character not found'; end if;

    update public.repoggle_progress
       set reward_claimed=true,updated_at=now()
     where user_id=v_user and not reward_claimed;
  else
    select gp,runecrafting_xp into v_new_gp,v_new_xp
      from public.characters where user_id=v_user;
  end if;

  return query select v_levels,v_gold,v_xp,v_new_gp,v_new_xp;
end;
$$;
...
grant execute on function public.claim_missing_repoggle_rewards() to authenticated;
...
comment on function public.complete_repoggle_level is 'Idempotently records a Repoggle completion and awards fixed server-side gold/Runecrafting XP once, including older unclaimed progress rows.';
comment on function public.claim_missing_repoggle_rewards is 'Safely repairs one-time rewards for older Repoggle progress rows whose reward_claimed flag is false.';
